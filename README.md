...
- ### Deliverables:
-  Account setup🗳️, list of free-tier limits📝, simple architecture diagram🏛️.
...
- **Labs:** Create IAM users and groups 👥,Attach policies to users/groups 📎, Create a role for EC2 (instance role) 🖥️, Test temporary credentials (STS) 🔑.
...
- **Topics:** AWS Lambda (serverless functions),  API Gateway (HTTP APIs), 🌐🔗S3 triggers (event-driven uploads) 🪣➡️⚡️, IAM roles for Lambda (execution role) , 🧾λCold startsand mitigation strategies, ❄️erverless best practices 🔧.
- **Labs:** Create a Lambda function triggered by S3 or API Gateway ⚙️ Test end-to-end flow (invoke → process → response) ✅
